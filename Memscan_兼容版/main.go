		...
		if mergeCheck.Checked {
			...
			for _, h := range filtered {
				if _, ok := groups[h.PID]; !ok {
					...
					pids = append(pids, h.PID) // 修复报错1：删除多余的pids参数
				}
			...
			for _, pid := range pids {
				...
				content := container.NewVBox() // 空容器，默认隐藏
				content.Hide()

				// 核心优化：点击展开时才创建UI，不提前渲染（解决卡顿）
...
func getNetworkMap() map[uint32][]string {
	...
	for i := 0; i < int(numEntries); i++ {
		...
		if row.RemoteAddr != 0 {
			rIP := make(net.IP, 4)
			binary.LittleEndian.PutUint32(rIP, row.RemoteAddr) // 修复报错2：Uint32 → PutUint32
